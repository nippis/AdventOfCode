                       ...
                       const std::pair<int, int>& coord)
{
  ...
  for (int row = coord.first-1; row >= 0; --row)
  {
    ...
    if (grid.at(row).at(coord.second).first >= max)
    {
      max = grid.at(row).at(coord.second).first;
    }
    scores.at(0)++;
  }
  max = -1;
  ...
  {
    if (max >= grid.at(coord.first).at(coord.second).first) break;
    if (grid.at(coord.first).at(col).first >= max)
    {
      max = grid.at(coord.first).at(col).first;
    }
    scores.at(2)++;
  }
  max = -1;
...
std::pair<std::string, std::string> day8(std::ifstream file)
{
  ...
  for (int row = 0; row < grid.size(); ++row)
  {
    ...
    {
      if (grid.at(row).at(column).first > max)
      {
        max = grid.at(row).at(column).first;
        if (!(grid.at(row).at(column).second))
        {
          grid.at(row).at(column).second = true;
          ++visible;
        }
      }
    }
